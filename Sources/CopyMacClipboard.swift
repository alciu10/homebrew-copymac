...
final class GlobalHotkeyManager: ObservableObject {
    ...
    private func setupEventHandler() {
        ...
        if status != noErr {
            print("InstallEventHandler failed: \(status)")
        }
    }
    ...
    func toggleAppVisibility() {
        DispatchQueue.main.async {
            self.isAppVisible ? self.hideApp() : self.showAppAtMouse()
        }
    }
    ...
    func registerHotkey(_ combo: String) -> Bool {
        guard let (keyCode, modifiers) = parseKeyCombo(combo) else {
            print("Invalid combo: \(combo)")
            return false
        }
        
        // Require at least one modifier for safety
        if modifiers == 0 {
            print("At least one modifier is required")
            return false
        }
        
        // Check for reserved shortcuts
        let reserved = ["⌘C", "⌘V", "⌘X", "⌘Z", "⌘Q", "⌘W", "⌘H", "⌘A", "⌘N", "⌘O", "⌘P", "⌘T", "⌘,"]
        if reserved.contains(where: { combo.hasPrefix($0) }) {
            print("Hotkey registration failed: \(combo) is a reserved shortcut")
            return false
        }
        
        // Check if already registered
        guard hotkeyRefs[combo] == nil else {
            print("Hotkey registration failed: \(combo) is already registered")
            return false
        }
    ...
    func unregisterHotkey(_ combo: String) {
        if let ref = hotkeyRefs[combo] {
            UnregisterEventHotKey(ref)
        }
        hotkeyRefs.removeValue(forKey: combo)
        if let id = comboToID.removeValue(forKey: combo) {
    ...
    func parseKeyCombo(_ combo: String) -> (keyCode: CGKeyCode, modifiers: UInt32)? {
        ...
        let canonical: String = {
            ...
            default:
                ...
                if up.hasPrefix("F"), let n = Int(up.dropFirst()), (1...19).contains(n) {
                    return "F\(n)"
                }
                return up
            }
    ...
    private func charToKeyCode(_ k: String) -> CGKeyCode? {
        let map: [String: CGKeyCode] = [
            ...
            "RETURN": 36, "L": 37, "J": 38, "'": 39, "K": 40, ";": 41, "\\": 42, ",": 43, "/": 44, "N": 45, "M": 46, ".": 47,
    ...
    // MARK: - Accessibility (Kept for compatibility)
    func checkAccessibilityPermission() -> Bool {
        // Since we removed accessibility dependency, we can always return true
        // or implement a basic check if needed for UI purposes
        return true
    }
